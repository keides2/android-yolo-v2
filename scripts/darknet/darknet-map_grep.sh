...
echo "mapファイルのCRをLFに置き換えます"
sed -e "s/\r/\n/g" backup/$TRIAL/$FILE_NAME'.txt' > backup/$TRIAL/$FILE_NAME'-A.txt'

# 必要な情報を抜き出す
echo "class, thresh, mAP を含む行抜き出します"
grep -e $TRIAL -e class -e thresh -e mAP backup/$TRIAL/$FILE_NAME'-A.txt' > backup/$TRIAL/$FILE_NAME'-B.txt'
